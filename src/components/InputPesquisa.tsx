...
type InputPesquisaProps = {
  setCartas: React.Dispatch<React.SetStateAction<CartaI[]>>
}
...
export function InputPesquisa({ setCartas }: InputPesquisaProps) {
  ...
  async function enviaPesquisa(data: Inputs) {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/cartas/pesquisa/${data.termo}`)
    const dados = await response.json()
    if (dados.length == 0) {
      toast.error("Não há cartas com o termo pesquisado")
      reset({termo:""})
      return
    }
    setCartas(dados)
  ...
  return (
    <section className="flex max-w-5xl mx-auto p-3">
      ...
      <form className="flex-1"
        ...
        <div className="relative">
          ...
          <input type="search" id="default-search" className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            placeholder="Qual jogador você procura? (nome, nacionalidade, ano de nascimento ou overall mínimo)"
            required
            {...register("termo")} />
